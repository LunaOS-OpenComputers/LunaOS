...
function GUI.actionButtons(x, y, fatSymbol)
	...
	container.close.colors.disabled.background = nil
	container.minimize.colors.disabled.background = nil
	container.maximize.colors.disabled.background = nil
...
function GUI.alert(...)
	...
	local sign = image.fromString([[06030000FF 0000FF 00F7FF▟00F7FF▙0000FF 0000FF 0000FF 00F7FF▟F7FF00 F7FF00 00F7FF▙0000FF 00F7FF▟F7FF00 F7FF00 F7FF00 F7FF00 00F7FF▙]])
	local offset = 2
...
function GUI.palette(x, y, startColor)
	...
	bigCrest.draw = function(object)
		-- paletteDrawBigCrestPixel(object.x + 2, object.y + 1, "⊙")
	...
	miniCrest.draw = function(object)
		screen.drawText(object.x, object.y, color.RGBToInteger(color.HSBToRGB(math.floor(palette.color.hsb.hue), 1, 1)) or 0x0, "▐")
		screen.drawText(object.x + 4, object.y, color.RGBToInteger(color.HSBToRGB(math.floor(palette.color.hsb.hue), 1, 1)) or 0x0, "▌")
	...
	local colorPanelText = palette:addChild(GUI.label(colorPanel.x, colorPanel.y, colorPanel.width, colorPanel.height, 0xFFFFFF, "")):setAlignment(GUI.ALIGNMENT_HORIZONTAL_CENTER, GUI.ALIGNMENT_VERTICAL_CENTER)
	...
	local function paletteRefreshBigImage()
		...
		for j = 1, bigImage.height do
			for i = 1, bigImage.width do
				image.set(bigImage.image, i, j, color.optimize(color.RGBToInteger(color.HSBToRGB(math.floor(palette.color.hsb.hue), saturation, brightness))), 0x0, 0x0, " ")
	...
	local function paletteRefreshMiniImage()
		...
		for j = 1, miniImage.height do
			...
			hueColorStep[j] = color.optimize(color.HSBToInteger(hue, 1, 1))
	...
	local function paletteUpdateInputs()
		local r, g, b = palette.color.rgb.red, palette.color.rgb.green, palette.color.rgb.blue
		...
		colorPanelText.colors.text = (r + g + b) / 3 >= 127 and 0x0 or 0xFFFFFF
		colorPanelText.text = (r + g + b) / 3 >= 127 and "B      B" or "W      W"
	end
	...
	local paletteConfigPath = paths.user.applicationData .. "LunaOS/GUI/Palette.cfg"
...
function GUI.menu(x, y, width, backgroundColor, textColor, backgroundPressedColor, textPressedColor, backgroundTransparency)
	...
	menu.addContextMenu = menuAddContextMenuItem
