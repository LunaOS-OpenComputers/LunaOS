...
local function addMainDesktopMenuItem(menu)
	...
	local apps = item:addSubMenuItem("📱", localization.apps)
	
	local function createAppViaCode(icon, name, code)
		local currapp = apps:addSubMenuItem(icon, name)

		currapp:addItem("⚡", localization.launch).onTouch = function()
			if type(code) == "function" then
				local succ, err = pcall(code)
				if not succ then
					GUI.alert(err)
				end
			else
				GUI.alert("Not a valid application")
			end
		end
	end
	
	local function createAppViaDir(icon, name, dir)
		local currapp = apps:addSubMenuItem(icon, name)
	...
	createAppViaCode("🎨", "Pallete", function()
